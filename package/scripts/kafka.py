...
def schema_registry(settings):
    import params
    schema_registry_config = {}
    schema_registry_config['schema_registry_http'] = params.config['configurations']['schema-registry-conf']['http.port']
    schema_registry_config['schema_registry_zk_url'] = params.config['configurations']['kafka-broker']['zookeeper.connect']
    schema_registry_config['schema_registry_topic'] = params.config['configurations']['schema-registry-conf']['schema.registry.topic']
    File(format("{schema_registry_conf}"),
       content=Template("schema-registry.properties.js", configurations = schema_registry_config),
       owner=params.kafka_user, group=params.user_group)
      

def connect(kafka_broker_config):
    import params
    connect_config = mutable_config_dict(params.config['configurations']['connect-conf'])
    connect_config['bootstrap.servers'] = build_broker_list(params.kafka_hosts, kafka_broker_config['listeners'])
    PropertiesFile("connect-npi.properties",
                      dir=params.conf_dir,
                      properties=connect_config,
                      owner=params.kafka_user,
                      group=params.user_group,
    )
    if (params.connect_log4j_props != None):
        File(format("{conf_dir}/connect-log4j.properties"),
             mode=0644,
             group=params.user_group,
             owner=params.kafka_user,
             content=params.connect_log4j_props
         )    

def build_broker_list(broker_host_list, listeners):
    broker_port = [ url[url.rindex(":")+1:] for url in listeners.split(",") ][0] #Assume all broker same port
    brokers = ["{0}:{1}".format(host, broker_port) for host in broker_host_list]
    return str(",".join(brokers))
